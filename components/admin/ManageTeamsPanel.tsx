...
export default function ManageTeamsPanel() {
  ...
  async function fetchTeams() {
    ...
    try {
      ...
      const data =
        (await response.json()) as AdminTeamsResponse;
  ...
  function handlePlayerImageChange(
    ...
    /*
     * Allows selecting the exact
     * same file again afterwards.
     */
    event.target.value = "";
  ...
  async function handleSave(
    ...
    if (
      ...
      setModalError(
        "Each player must have a name, number, and image."
      );
      ...
      cleanedPlayers.map(
        (
          ...
          return {
            ...
            /*
             * Existing Supabase URL is sent
             * when the image was not changed.
             *
             * If a new imageFile exists,
             * the API replaces this value
             * with the newly uploaded
             * Supabase URL.
             */
            image:
              player.image,
    ...
    try {
      ...
      try {
        data =
          (await response.json()) as SingleTeamResponse;
      } catch {
        throw new Error(
      ...
      if (
        ...
        throw new Error(
          "The server did not return the saved team."
        );
      }
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-white/50">
                Create, edit, publish, or delete national team sections and players.
              </p>
            </div>
          </div>
                                ...
                                <label className="flex cursor-pointer items-center justify-center gap-3 rounded-[14px] border border-dashed border-white/[0.16] bg-white/[0.05] px-4 py-3 transition hover:border-[#00c896]/55 hover:bg-[#00c896]/[0.06]">
                                  {player.previewUrl ? (
                                    <img
                                      ...
                                      alt={`${player.name || `Player ${index + 1}`} preview`}
